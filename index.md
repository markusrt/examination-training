...
{% assign exams = site.pages | where: "typ", "klausur" | sort: "fach" %}
{% for exam in exams %}
  {% assign loesung_url = exam.url | replace: 'klausur-', 'loesung-' %}
  {% assign klausur_pdf = exam.url | replace: '.html', '.pdf' %}
  {% assign loesung_pdf = loesung_url | replace: '.html', '.pdf' %}
  <div class="exam-card">
    <h3>{{ exam.fach }} – Nr. {{ exam.nr }}</h3>
    <p>
      📍 {{ exam.bundesland }} · 🎓 {{ exam.klassenstufe }}<br>
      {% if exam.bearbeitungszeit %}⏱️ {{ exam.bearbeitungszeit }} · {% endif %}
      {% if exam.gesamtpunktzahl %}📊 {{ exam.gesamtpunktzahl }}{% endif %}
    </p>
    <div class="card-links">
      <a class="btn btn-outline" href="{{ exam.url | relative_url }}">📋 Klausur</a>
      <a class="btn btn-outline" href="{{ loesung_url | relative_url }}">📝 Lösung</a>
      <a class="btn btn-primary" href="{{ klausur_pdf | relative_url }}">⬇ Klausur PDF</a>
...
{% if exams.size == 0 %}
_Noch keine Klausuren vorhanden._
{% endif %}
...
- **Aufgabenblatt** (`klausur-XX.md`): Enthält die Aufgaben mit Bewertungseinheiten (BE).
- **Musterlösung** (`loesung-XX.md`): Vollständige Rechenwege mit LaTeX-Formeln und Diagrammen.
- **PDF-Download**: Jede Klausur und Lösung steht als druckfertiges PDF bereit.
